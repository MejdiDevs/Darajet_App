...
سلسلة الأيام: ${streakDays}`;
    ...
    try {
        ...
        return {
            ...
            encouragement_ar: `لا تيأس يا ${userName}! كل محاولة هي خطوة للتعلم. تذكر أن ثواب قراءة وتدبر القرآن عظيم.`,
        };
    }
}
...
export async function evaluateRoadmap({ questions, answers, userLevel = 'مبتدئ' }) {
    ...
    try {
        ...
        return {
            overall_score: 5,
            per_question_scores: questions.map(() => 5),
            per_question_corrections: questions.map(() => 'الإجابة غير واضحة أو لا علاقة لها بالسؤال المطروح.'),
